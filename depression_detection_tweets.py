...
tweets = pd.read_csv('sentiment_tweets5.csv',encoding='mac_roman')
tweets.head(20)
...
def process_message(message, lower_case = True, stem = True, stop_words = False, gram = 3):
    if lower_case:
...
#joblib.dump(sc_tf_idf,'predicted.pkl')
dump(sc_tf_idf, filename="dep_class.joblib")
"""# Predictions with TF-IDF

# Depressive Tweets
"""

pm = process_message('Feeling Happy but uneasy')
...
pm = process_message('Someone help, I am depressed')
sc_tf_idf.classify(pm)
...
pm = process_message('Nothing productive.')
sc_tf_idf.classify(pm)

pm = process_message('My depression will not let me work out')
sc_tf_idf.classify(pm)

pm = process_message('John is depressed')
sc_tf_idf.classify(pm)
